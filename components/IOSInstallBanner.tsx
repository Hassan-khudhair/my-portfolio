...
function isIOS(): boolean {
  ...
  // iPad on iOS 13+ reports as Mac + has touch support
  return /Mac/.test(navigator.userAgent) && navigator.maxTouchPoints > 1;
}

function isSafariOnly(): boolean {
  if (typeof navigator === 'undefined') return false;
  // Exclude browsers on iOS that cannot install PWAs
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-100 animate-fade-in-up">
        ...
        {/* "Tap share button" hint */}
        <div className="flex justify-center mb-2">
          <div className="flex flex-col items-center gap-1">
            <p className="text-xs text-white font-semibold bg-black/60 rounded-full px-3 py-1 backdrop-blur-sm">
        ...
        <div className="mx-3 mb-6 rounded-2xl overflow-hidden shadow-2xl border border-white/10">
          ...
          <div className="bg-white dark:bg-zinc-900 px-5 py-5 space-y-4">
            ...
            >
              The square-with-arrow button at the bottom of Safari
            </Step>
            ...
            >
              The app will appear on your Home Screen with its icon
            </Step>

            <button
              onClick={dismiss}
              className="w-full py-3 rounded-xl text-sm font-semibold text-gray-400 dark:text-gray-500 hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors"
            >
              Maybe later
            </button>
          </div>
